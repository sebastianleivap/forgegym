...
export default function AuthScreen({ onAuth }) {
  ...
  const handleLogin = async () => {
    ...
    if (err) {
      setLoading(false)
      if (err.message.includes('Email not confirmed')) setError('Debes confirmar tu email. Revisa tu bandeja de entrada.')
    ...
    if (profErr || !profile) {
      ...
      setError('Tu cuenta no tiene perfil. Por favor crea una cuenta nueva.')
      return
    }
  ...
  const handleRegister = async () => {
    ...
    if (err) {
      setLoading(false)
      if (err.message.includes('already registered')) setError('Este email ya está registrado. Intenta iniciar sesión.')
      else setError(err.message)
      return
    }
    if (!data.user) { setLoading(false); setError('No se pudo crear la cuenta. Intenta de nuevo.'); return }
    const { error: insertErr } = await supabase.from('profiles').insert({
      id: data.user.id, name: name.trim(), email: email.trim().toLowerCase(), role,
      gym_name: role === 'admin' ? gymName : null,
    })
    setLoading(false)
    if (insertErr) { setError('Error al guardar el perfil: ' + insertErr.message); return }
    if (data.session) {
      onAuth(data.user, { id: data.user.id, name: name.trim(), email, role, gym_name: gymName || null })
      return
    }
          ...
          <>
            ...
            {role === 'admin' && (
              <div className="fg">
                ...
                <input placeholder="Ej: CrossFit Concepción" value={gymName} onChange={e => setGymName(e.target.value)} onKeyDown={handleKey} />
              </div>
            )}
          </>
        )}
